...
class EnhancedTable extends React.Component {
  ...
  handleMenuFiltros = (estatus) => {
    ...
    if (stepper===0){
      ...
      console.log('antes de filtar ',this.props.data);
       datosTabla = this.props.data.filter((item) => item.Activo == estatus);
       console.log('despues de filtar',datosTabla);
  ...
  verDetalleModulo=(idFila,activo) =>{
    const { setStepperAction,setTextfieldmoduloTextAction,
      setIdmoduloAction,setTextfielddescripcionTextAction,filterData,setModulosdisableAction,setNombremodulosinactualizarAction } = this.props;
    ...
    console.log('dter',datosModulos);
    setNombremodulosinactualizarAction(datosModulos[0].NombreModulo);
  ...
  verDetalleFuncion=(idFila) =>{
    const { setStepperaddmoduloAction,setSelectedAction,setActualizafuncionAction,
      setModalfuncionesdisableAction } = this.props;
    if(idFila!=null){
      setSelectedAction([idFila]);
      setModalfuncionesdisableAction(true);
      setActualizafuncionAction(true);
      setStepperaddmoduloAction(true);
    }
  }

  actualizarFunciones = (idFila) => {
    const { setStepperaddmoduloAction,setSelectedAction,setActualizafuncionAction } = this.props;
    if(idFila!=null){
      setSelectedAction([idFila]);
      setActualizafuncionAction(true);
      setStepperaddmoduloAction(true);
                        ...
                        { selected.length < 2 &&
                          (
                            <div className="DataTableopciones">
                              { download === true && permisos.especiales.descargararchivo 
                                ? <Tooltip
                                  title="Descargar"
                                  placement="bottom"
                                >
                              ...
                              {stepper===0 || !n.Activo ?
                                <Tooltip
                                  ...
                                  placement="bottom"
                                >
                                  <IconButton aria-label="Ver detalle" onClick={()=> soloId ? onEditar(n[idFila]) : (stepper===0 ? this.verDetalleModulo(n[idFila],n.Activo): this.verDetalleFuncion(n[idFila]))}>
                                ...
                                <Tooltip
                                  ...
                                  placement="bottom"
                                >
                                  <IconButton aria-label="Editar" onClick={()=>this.actualizarFunciones(n[idFila])}>
                                    <img src={EditIconModulo} style={{width: 16}} alt="Editar" />                                  
                                  </IconButton>
                                </Tooltip>
                              }
                              { n.Activo && permisos.normales.eliminar ?
                                <Tooltip
                                  title="Eliminar"
                                  placement="bottom"
                                >
...
EnhancedTable.propTypes = {
  ...
  actualizarFunciones: PropTypes.func,
