...
function asignacionEncuestasReducer(state = initialState, action) {
  switch (action.type) {
    case DEFAULT:{
      return state;
    }
    ...
    case ASIGNAR_USUARIOS: {
      ...
      return state.setIn(['asignacionTabla', 'nuevaAsignacion', 'stepper'], 1)
        .setIn(['asignacionTabla', 'collapse'], true)
    ...
    case ON_CHANGE_DEPARTAMENTOS: {
      const asignaciones = []
      const asignacionesExistentes = state.getIn(['asignacionTabla', 'nuevaAsignacion', 'tablas', 'asignaciones', 'datos']).toJS()
      const usuarios = state.getIn(['asignacionTabla', 'nuevaAsignacion', 'combos', 'usuarios']).toJS()
      const departamentos = state.getIn(['asignacionTabla', 'nuevaAsignacion', 'combos', 'departamentos']).toJS()
      const puestos = state.getIn(['asignacionTabla', 'nuevaAsignacion', 'combos', 'puestos']).toJS()
      const seleccionados = []
      ...
      if(action.seleccionado.length > 0) {
        if(action.seleccionado[0].value === -1) {
          ...
          asignacionesFilter = asignacionesExistentes.filter(el => lista.some(elem => elem.value === el.IdDepartamento));
        }
      } else {
...
export default asignacionEncuestasReducer;
