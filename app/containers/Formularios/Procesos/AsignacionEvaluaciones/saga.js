...
export function* getDatosGeneralesAction() {
  ...
  try {
    ...
    const datosGenerales = {
      formularios: forms.filter(registro => registro.referencia === 'REFEVA'),
      asignaciones: asignaciones.data.filter(registro => registro.TipoFormulario === 'REFEVA'),
...
export function* getFormularioDetalleAction(action){
  ...
  const departamentos = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'combos', 'departamentos']).toJS());
  const puestos = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'combos', 'puestos']).toJS());
  const formularios = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'combos', 'formularios']).toJS());
  const usuarios = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'combos', 'usuarios']).toJS());
...
export function* postAsignacionAction(){
  ...
  const nuevaAsignacion = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion']).toJS());
  const global = yield select((state) => state.getIn(['global']).toJS()); 
  const {
    campos,
    tablas: {
...
export function* updateAsignacionAction(){
  ...
  const nuevaAsignacion = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion']).toJS());
  const global = yield select((state) => state.getIn(['global']).toJS());
  const idAsignacion = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'idAsignacion']))
...
export function* checkUserAction(action){
  ...
  try {
    ...
    const sttIdAsignacion = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'asignacionSlc']))
    const asignaciones = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'tablas', 'asignaciones', 'datos']).toJS());
    const stepper = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'stepper']));
    let asignacion 
    if(stepper === 0) {
    ...
    const idFormulario = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'nuevaAsignacion', 'campos', 'formulario', 'valor']).toJS());
    let idAsignacionDetalle = yield select((state) => state.getIn(['asignacionEvaluaciones','asignacionTabla', 'idAsignacion']));
...
// Individual exports for testing
export default function* asignacionEvaluacionesSaga() {
  yield[
    takeLatest(
      OBTENER_PERMISOS,
