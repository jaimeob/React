...
export function* getDatosGeneralesAction() {
  try {
    ...
    const forms = formularios.data.map(el=> ({ value:el.IdFormulario, label:el.Nombre.trim(), referencia: el.Referencia, requiereValidacion: el.RequiereValidacion }));
    ...
    const datosGenerales = {
      formularios: forms.filter(registro => registro.referencia === 'REFFOR'),
      asignaciones: asignaciones.data.filter(registro => registro.TipoFormulario === 'REFFOR'),
...
export function* getFormularioDetalleAction(action){
  ...
  const departamentos = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'combos', 'departamentos']).toJS());
  const puestos = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'combos', 'puestos']).toJS());
  const formularios = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'combos', 'formularios']).toJS());
  const usuarios = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'combos', 'usuarios']).toJS());
  ...
  // 012 730 01531354152 5
  const nuevaAsignacion = {
    tipoFormulario: data[0][0].Referencia,
    stepper: 0,
    requiereValidacion: data[0][0].RequiereValidacion,
    campos: {
      descripcion: {
...
export function* postAsignacionAction(){
  ...
  const nuevaAsignacion = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion']).toJS());
  const requiereValidacion = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'requiereValidacion']));
  const global = yield select((state) => state.getIn(['global']).toJS()); 
  const {
    campos,
    tablas: {
...
export function* updateAsignacionAction(){
  ...
  const requiereValidacion = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'requiereValidacion']));
  const global = yield select((state) => state.getIn(['global']).toJS());
  const idAsignacion = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'idAsignacion']))
...
export function* checkUserAction(action){
  try {
    ...
    const sttIdAsignacion = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'asignacionSlc']))
    const asignaciones = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'tablas', 'asignaciones', 'datos']).toJS());
    const stepper = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'stepper']));
    let asignacion 
    if(stepper === 0) {
      // eslint-disable-next-line prefer-destructuring
      asignacion = asignaciones.filter(registro => IdAsignacion === registro.IdAsignacion)[0]
    } else {
      // eslint-disable-next-line prefer-destructuring
      asignacion = asignaciones.filter(registro => sttIdAsignacion + 1 === registro.IdAsignacion)[0]
    }
    const idFormulario = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'nuevaAsignacion', 'campos', 'formulario', 'valor']).toJS());
    let idAsignacionDetalle = yield select((state) => state.getIn(['asignacionFormulario','asignacionTabla', 'idAsignacion']));
    ...
    if(status === 200){
      if(stepper === 0) {
        ...
        yield put(
          Actions.get('REMOVE_ROW_USUARIOS').fn(IdAsignacion, data, bandera),
        );
      }
...
// Individual exports for testing
export default function* aasignacionFormularioSaga() {
  yield[
    takeLatest(
      OBTENER_PERMISOS,
