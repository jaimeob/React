...
function MultiValue(props) {
  return (
    <Chip
      ...
      onDelete={props.onDelete || props.removeProps.onClick}
      size='small'
...
function Menu(props) {
  return (
    <Paper style={{zIndex: 1000}} square className={props.selectProps.classes.paper} {...props.innerProps}>
      {props.children}
    </Paper>
  );
}
  ...
  return (
    <React.Fragment>
      <Select
        ...
        requeridoDesactivo={requeridoDesactivo}
        TextFieldProps={
          {
            label: ((!multiple && valor !== '') || (multiple && valor.length > 0)) ? label : '' ,
            InputLabelProps: {
              htmlFor: label,
              shrink: true,
              style: {
        ...
        classes={classes}
        styles={selectStyles}
        placeholder={opciones.length ? label : props.labelVacio}
      />
    </React.Fragment>
  );
}

FiltroSeleccion.defaultProps = {
  labelVacio: 'No hay información',
}
